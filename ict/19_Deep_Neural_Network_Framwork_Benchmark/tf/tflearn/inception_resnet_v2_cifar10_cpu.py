...
Applying 'inception_resnet_v2' to CIFAR-10 dataset classification task.
...
model = tflearn.DNN(network, checkpoint_path='checkpoint_cifar10/inception_resnet_v2',
                    max_checkpoints=1, tensorboard_verbose=2, tensorboard_dir="/tflearn_logs/")
model.fit(X, Y, n_epoch=500, validation_set=(testX, testY), shuffle=True,
          show_metric=True, batch_size=32, snapshot_step=200,
          snapshot_epoch=False, run_id='inception_resnet_v2_cifar10')
		  
model.save('model/cifar10_retrained_by_inception_resnet_v2')